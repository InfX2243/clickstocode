...
import { useElementScrollProgress, useReducedMotion, useScrollProgress } from '../lib/useScrollProgress';
...
function ChapterShell({ eyebrow, title, children, progress, visibility }: { eyebrow: string; title: string; children: ReactNode; progress: number; visibility: number }) {
  return (
    <CinematicChapter eyebrow={eyebrow} title={title} progress={progress} visibility={visibility}>
      {children}
    </CinematicChapter>
  );
}
...
export default function Home() {
  ...
  const storyProgress = useElementScrollProgress(storyRef);
  ...
  return (
    <div className="experience">
      ...
      <section ref={storyRef} className="cinematic-story-stage" aria-label="AWS From Clicks to Code story">
        <div className="cinematic-story-frame">
          <ChapterShell eyebrow="THE JOURNEY" title="What are we actually doing?" progress={chapterProgresses[0]} visibility={chapterVisibilities[0]}>
            <div className="chapter-split"><p className="scene-copy">Start with the console. End with infrastructure you can describe, repeat, and ship.</p><div className="motif" aria-label="Click to code journey"><span>CLICK</span><i>→</i><span className="accent">CODE</span></div></div>
          </ChapterShell>
          <ChapterShell eyebrow="THE CLICK" title="One action. One server." progress={chapterProgresses[1]} visibility={chapterVisibilities[1]}>
            <div className="console-scene" style={{ transform: `translateY(${(1 - chapterProgresses[1]) * 18}px) scale(${0.96 + chapterProgresses[1] * 0.04})` }}><div className="console-bar"><span className="dot" /><span>EC2 / Instances</span><span className="console-status">READY</span></div><div className="console-body"><div className="console-nav"><b>Compute</b><span>Instances</span><span>Images</span><span>Security</span></div><div className="console-main"><span className="console-kicker">INSTANCE ACTION</span><strong>Launch instance</strong><span className="console-button" aria-hidden="true">Launch</span><span className="fake-cursor" aria-hidden="true" style={{ transform: `translate(${chapterProgresses[1] * 150}px, ${chapterProgresses[1] * 55}px)` }}>➤</span></div></div></div>
          </ChapterShell>
          <ChapterShell eyebrow="THE PROBLEM" title="What if infrastructure could remember what you wanted?" progress={chapterProgresses[2]} visibility={chapterVisibilities[2]}>
            <div className="repeat-scene">{Array.from({ length: 5 }, (_, i) => <div key={i} className="repeat-card" style={{ transform: `translateX(${i * chapterProgresses[2] * 22}px) translateY(${i * chapterProgresses[2] * 8}px)`, opacity: 0.45 + i * 0.11 }}><span>Launch instance</span><small>manual action</small></div>)}</div>
          </ChapterShell>
          <ChapterShell eyebrow="THE TURN" title="From clicking to declaring." progress={chapterProgresses[3]} visibility={chapterVisibilities[3]}>
          ...
          <ChapterShell eyebrow="INFRASTRUCTURE AS CODE" title="Human → Code → Plan → Infrastructure" progress={chapterProgresses[4]} visibility={chapterVisibilities[4]}>
          ...
          <ChapterShell eyebrow="THE PIPELINE" title="Click → Console → Code → Plan → Apply → Infrastructure" progress={chapterProgresses[5]} visibility={chapterVisibilities[5]}>
          ...
          <ChapterShell eyebrow="YOUR MISSION" title="Build the muscle, not just the demo." progress={chapterProgresses[6]} visibility={chapterVisibilities[6]}>
            <div className="mission-grid">{[['Provision','Create repeatable infrastructure'],['Secure','Remove fragile access paths'],['Automate','Make the next run predictable']].map(([t,d], i) => <article key={t} className="mission-card" style={{ opacity: chapterProgresses[6] >= i / 3 ? 1 : .25, transform: `translateY(${chapterProgresses[6] >= i / 3 ? 0 : 18}px)` }}><strong>{t}</strong><span>{d}</span></article>)}</div>
          </ChapterShell>
          <ChapterShell eyebrow="MISSION RUNTIME" title="The lab, as an execution log." progress={chapterProgresses[7]} visibility={chapterVisibilities[7]}>
          ...
          <ChapterShell eyebrow="$ WHOAMI" title="Meet the builder." progress={chapterProgresses[8]} visibility={chapterVisibilities[8]}>
          ...
          <ChapterShell eyebrow="WHY IT MATTERS" title="You're not just learning AWS." progress={chapterProgresses[9]} visibility={chapterVisibilities[9]}>
          ...
          <ChapterShell eyebrow="PRE-FLIGHT" title="Before you enter the lab." progress={chapterProgresses[10]} visibility={chapterVisibilities[10]}>
