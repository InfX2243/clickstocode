...
const chapters = 11;
const STORY_SCROLL_UNITS = 22;
const TOTAL_SCROLL_UNITS = 24;
const clamp01 = (value: number) => Math.min(1, Math.max(0, value));
const chapterProgress = (global: number, index: number) => {
  const story = clamp01(((global * TOTAL_SCROLL_UNITS) - 1) / STORY_SCROLL_UNITS);
  return clamp01(story * chapters - index);
};

function ChapterShell({ eyebrow, title, children, progress }: { eyebrow: string; title: string; children: ReactNode; progress: number }) {
  return (
    <CinematicChapter eyebrow={eyebrow} title={title} progress={progress}>
      {children}
    </CinematicChapter>
  );
}
...
export default function Home() {
  ...
  return (
    <div className="experience">
      ...
      <ChapterShell eyebrow="THE JOURNEY" title="What are we actually doing?" progress={chapterProgresses[0]}>
      ...
      <ChapterShell eyebrow="THE CLICK" title="One action. One server." progress={chapterProgresses[1]}>
      ...
      <ChapterShell eyebrow="THE PROBLEM" title="What if infrastructure could remember what you wanted?" progress={chapterProgresses[2]}>
      ...
      <ChapterShell eyebrow="THE TURN" title="From clicking to declaring." progress={chapterProgresses[3]}>
      ...
      <ChapterShell eyebrow="INFRASTRUCTURE AS CODE" title="Human → Code → Plan → Infrastructure" progress={chapterProgresses[4]}>
      ...
      <ChapterShell eyebrow="THE PIPELINE" title="Click → Console → Code → Plan → Apply → Infrastructure" progress={chapterProgresses[5]}>
      ...
      <ChapterShell eyebrow="YOUR MISSION" title="Build the muscle, not just the demo." progress={chapterProgresses[6]}>
      ...
      <ChapterShell eyebrow="MISSION RUNTIME" title="The lab, as an execution log." progress={chapterProgresses[7]}>
      ...
      <ChapterShell eyebrow="$ WHOAMI" title="Meet the builder." progress={chapterProgresses[8]}>
      ...
      <ChapterShell eyebrow="WHY IT MATTERS" title="You're not just learning AWS." progress={chapterProgresses[9]}>
      ...
      <ChapterShell eyebrow="PRE-FLIGHT" title="Before you enter the lab." progress={chapterProgresses[10]}>
